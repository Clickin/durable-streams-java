...
const baseUrl = process.env.STREAM_URL ?? "http://localhost:4436";

function startServer() {
  if (isWindows) {
    return spawn("cmd.exe", ["/c", gradlew, ":durable-streams-conformance-runner:runFileConformanceServer", "--no-daemon"], {
      stdio: "inherit",
      cwd: rootDir,
    });
  }
  return spawn(gradlew, [":durable-streams-conformance-runner:runFileConformanceServer", "--no-daemon"], {
    stdio: "inherit",
    cwd: rootDir,
  });
}
