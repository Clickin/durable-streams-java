...
const baseUrl = process.env.STREAM_URL ?? "http://127.0.0.1:4435";

function startServer() {
  if (isWindows) {
    return spawn("cmd.exe", ["/c", gradlew, ":example-ktor:run", "--no-daemon"], {
      stdio: "inherit",
      cwd: rootDir,
    });
  }
  return spawn(gradlew, [":example-ktor:run", "--no-daemon"], {
    stdio: "inherit",
    cwd: rootDir,
  });
}
